...
function organizarEmails(regrasSelecionadas) {
  ...
  Logger_.info("Regras: " + rulesToRun.map(function(r) { return r.name; }).join(", "));
...
'    h1{font-size:21px;color:#1a1a2e}\n' +
'    .version{font-size:11px;background:#f1f5f9;color:#64748b;padding:2px 8px;border-radius:20px;font-weight:600}\n' +
...
'    .rule-item{display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #f1f5f9}\n' +
...
'  </div>\n' +
'  <p class="sub">Inbox não lidos · Google Apps Script</p>\n' +
...
'      if(!names||!names.length){box.innerHTML="<span style=\\"color:#f87171\\">Nenhuma regra.</span>";return;}\n' +
'      box.innerHTML=names.map(function(n,i){\n' +
'        return "<div class=\\"rule-item\\"><input type=\\"checkbox\\" id=\\"r"+i+"\\" value=\\""+n+"\\" checked>"+\n' +
...
'    .getRules();\n' +
'\n' +
'  function getSelected(){return Array.from(document.querySelectorAll(".rule-item input:checked")).map(function(cb){return cb.value;});}\n' +
'\n' +
'  function rodar(){\n' +
'    var sel=getSelected();\n' +
...
'        "<span class=\\"tag "+(match?"tag-match":"tag-ai")+"\\">"+(match?t.regra:"→ IA geral")+"</span>"+\n' +
